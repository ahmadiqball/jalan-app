...
<template>
  ...
  >
    ...
    <div class="ml-auto flex items-center gap-3">
      ...
      <CoreButton variant="primary" class="!px-[18px] !py-[10px] !text-[14px]" @click="ui.showNewTrip = true">
        <i class="i-lucide-plus" />
        <span class="hidden sm:inline">Trip baru</span>
      </CoreButton>
      <button
        class="w-9 h-9 rounded-full bg-sand-100 hover:bg-sand-line flex items-center justify-center text-[13px] font-700 text-ink-2 transition-colors"
        title="Keluar"
        @click="signOut"
      >
        {{ session.initials }}
